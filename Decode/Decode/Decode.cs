    ...
    public class Decode
    {
        public String TIFF(string path)
        {
            try
            {
                ...
                for (int i = 200; i < source.Width; i = i + 10)
                {
                    for (int j = 200; j < source.Height; j = j + 10)
                    {
                        imgw = i; imgh = j;

                        Bitmap bmp = source.Clone(new System.Drawing.Rectangle(source.Width - imgw, source.Height - imgh, 175, 200), source.PixelFormat);

                        Result result = reader.Decode(bmp);

                        if (result != null&& result.Text.Split('_').Length==6)
                        {
                            cash = result.Text;
